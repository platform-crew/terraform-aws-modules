...
data "aws_iam_policy_document" "alb_assume_role" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:${var.namespace}:${var.cluster_name}-alb-sa"]
    }
  }
}

resource "aws_iam_role" "alb_controller_iam_role" {
  name               = "${var.cluster_name}-alb-iam-role"
...
# ServiceAccount for AWS Load Balancer Controller with IRSA enabled
resource "kubernetes_service_account" "aws_alb_controller_sa" {
  metadata {
    name      = "${var.cluster_name}-alb-sa"
...
resource "helm_release" "aws_alb_controller" {
  ...
  values = [
    yamlencode({
      ...
      serviceAccount = {
        create = false
        name   = kubernetes_service_account.aws_alb_controller_sa.metadata[0].name
      }
      region = var.region
      vpcId  = var.vpc_id
    })
  ...
  depends_on = [
    kubernetes_service_account.aws_alb_controller_sa,
